...
public class RulerView extends SurfaceView implements Callback {
    ...
    private void init(Context context) {
        ...
        RADIUS_BIG = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 46, dm);
        RADIUS_MEDIUM = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, dm);
        RADIUS_SMALL = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, dm);
        CYCLE_WIDTH = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, dm);
        DISPLAY_SIZE_BIG = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 40, dm);
        DISPLAY_SIZE_SMALL = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, dm);
        UNIT_MM = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_MM, 1, dm);
        RULE_HEIGHT = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30, dm);
        ...
        paint = new Paint();
        paint.setColor(mode.getColorPrimary(context));
        linePaint = new Paint();
        linePaint.setColor(mode.getColorPrimary(context));
        linePaint.setStrokeWidth(4);
        fontPaint = new Paint();
        fontPaint.setTextSize(FONT_SIZE);
        fontPaint.setAntiAlias(true);
        fontPaint.setColor(mode.getColorPrimary(context));
        lineX = PADDING;
        kedu = 0;
    }
    ...
    private void drawDisplay(Canvas canvas) {
        ...
        cyclePaint.setColor(mode.getColorPrimary(getContext()));
        cyclePaint.setAntiAlias(true);
        cyclePaint.setStyle(Paint.Style.FILL);
        strokPaint = new Paint();
        strokPaint.setAntiAlias(true);
        //中间大圆
        strokPaint.setColor(mode.getColorPrimary(getContext()));
        strokPaint.setAlpha(100);
        ...
        strokPaint.setColor(0xffffffff);
