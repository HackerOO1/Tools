...
import com.nice.tools.R;
import com.nice.tools.Tool_audio_Activity;
...
public class RoundIndicatorView extends View {
    ...
    private String[] text = {"安静", "良好", "中等", "嘈杂", "全损"};
    ...
    public void setCurrentNumAnim(int num) {
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                Tool_audio_Activity.audio_bg.setBackgroundColor(color);
            }
        });
        anim.start();
    }
    ...
    private int calculateColor(int value) {
        ...
        if (value <= maxNum / 2) {
            ...
            color = (int) evealuator.evaluate(fraction, 0xFF00CED1, 0xFFFF8C00); //由红到橙
        } else {
            fraction = ((float) value - maxNum / 2) / (maxNum / 2);
            color = (int) evealuator.evaluate(fraction, 0xFFFF8C00, 0xFFFF6347); //由橙到蓝
        }
        return color;
    }
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        maxNum = array.getInt(R.styleable.RoundIndicatorView_maxNum, 120);
        //下面是角度
    ...
    private void drawCenterText(Canvas canvas) {
        ...
        canvas.drawText(currentNum + "DB", -paint_4.measureText(currentNum + "DB") / 2, 0, paint_4);
        paint_4.setTextSize(radius / 7);
        String content = "环境噪音";
...
//        paint.getTextBounds(text,0,text.length(),rect);
        canvas.drawText(text, -width / 2, -radius + dp2px(23), paint);
        paint.setStyle(Paint.Style.STROKE);
    }
